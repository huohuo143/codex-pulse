...
`Codex 脉动 2.10.3` 继续提供 7 款 WidgetKit 小组件。所有组件读取主 App 原子写入的同一份聚合快照，数据口径与悬浮框一致，并随 macOS 白天/夜晚外观自动适配背景。总览和额度组件可显示 7 天外环与橙色 5 小时内环；小组件的 5 小时内环默认关闭，可在主 App 设置中独立开启。

## 组件组

| 小组件 | 支持尺寸 | 主要信息 |
| --- | --- | --- |
| Codex 算力总览 | 中、大 | 7 天额度外环、可选 5 小时内环、滚动 24h、重置概率、Full reset；大尺寸增加 Token 汇总、费用和 24h 趋势 |
| Codex 额度 | 小、中 | 7 天额度外环、可选 5 小时内环、已用比例、重置倒计时 |
...
- 悬浮框中的 7 天额度、5 小时额度、滚动 24h Token、重置雷达与 Full reset 权益可以分别勾选，至少保留一项；同时开启两种额度时显示为同心环。
- “设置 → macOS 桌面小组件 → 显示 5 小时额度内环”独立控制总览和额度小组件，默认关闭。
- 桌面小组件和悬浮框共享同一数据口径，但显示开关相互独立：悬浮框是否显示 5 小时额度不会改变小组件设置。

## 数据与刷新

- 主 App 继续按设置的 5/10/30 秒频率读取本地额度；完整 Token 汇总按现有 5 分钟节流执行。
- 重置雷达继续独立按 30 分钟同步公开源。
- 成功刷新后同时保留主 App 快照，并原子写入 Widget 扩展自身容器中的 `widget-snapshot.json`；schema 2 新增 5 小时额度及显示开关，并兼容旧快照。
