...
`Codex 脉动 2.10.1` 继续提供 7 款 WidgetKit 小组件。所有组件读取主 App 原子写入的同一份聚合快照，数据口径与悬浮框一致，并随 macOS 白天/夜晚外观自动适配背景。新增的 5 小时额度目前仅作为可选悬浮框指标，不改变 Widget schema；Widget 继续读取 `daily14`，可靠性事件、备份和每日摘要均保持不变。
...
- 主 App 首次发布快照时立即请求重载；之后仅在展示内容实质变化时请求，10 秒窗口内的多次变化会合并到最早可刷新时刻。
- 主 App 运行时，主面板数据变化后的 Widget 刷新请求目标为约 10 秒；系统仍会按桌面组件预算决定最终绘制时刻。
