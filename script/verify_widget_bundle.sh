...
APP_BUNDLE="${1:-$ROOT_DIR/dist/build-v2.10.0-build2101/Codex 脉动.app}"
...
assert_equal "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$APP_INFO")" "2.10.0" "App version"
assert_equal "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$APP_INFO")" "2101" "App build"
...
assert_equal "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$WIDGET_INFO")" "2.10.0" "Widget version"
assert_equal "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$WIDGET_INFO")" "2101" "Widget build"
...
echo "Verified 7 widget configurations, icons, standard sandbox, frameworks and signatures."
